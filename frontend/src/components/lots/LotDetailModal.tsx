// src/components/lots/LotDetailModal.tsx
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold">Lot Details</h2>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="mt-1 text-lg font-semibold">{data.lot_number || 'N/A'}</p>
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="mt-1">{data.manufacturing_date ? new Date(data.manufacturing_date).toLocaleDateString() : 'N/A'}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-500">Expiry Date</label>
              <p className="mt-1">{data.expiry_date ? new Date(data.expiry_date).toLocaleDateString() : 'N/A'}</p>
            </div>
          </div>
